...
@section('content')
    <div class="container">
        <div class="row">
            <div class="col-md-3"><h1 style="color:#888888">edit<span style="color:#00d053">challenge</span></h1>
            </div>
        </div>
    </div>
    ...
    <div class="container-fluid">
        <div class="row">
            <form class="form-horizontal" method="POST" action="{{action('ChallengesController@update', $challenge->id)}}"
                  id="user_parameters">
                {{ method_field('PUT') }}
                {!! csrf_field() !!}
                <div class="form-group" style="margin-right: 0px; margin-left: 0px;">
                        ...
                        <div class="col-sm-10">
                            <div class="btn-group" data-toggle="buttons">
                                @foreach($betTypes as $index => $betType)
                                    <span style="padding-right: 0" data-toggle="tooltip"
                                          title="{{$betType->description}}"><label id="{{$betType->name . 'Radio'}}"
                                                                                   class="btn btn-primary {{ $index == 0 ? 'active' : '' }}"><input
                                                    style="visibility: hidden; margin-left: -10px"
                                                    {{ $index == 0 ? 'checked' : '' }} type="radio"
                                                    data-toggle="tab" name="bet_type"
                                                    autocomplete="off"
                                                    value="{{$betType->id}}">{{$betType->name}}</label></span>
                                @endforeach
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-10">
                            <div class="btn-group" data-toggle="buttons">
                                @foreach($challengeTypes as $index => $challengeType)
                                    <span><label class="btn btn-primary {{ $index == 0 ? 'active' : '' }}"><input
                                                    style="visibility: hidden; margin-left: -10px"
                    ...
                    <div id="targetScore" class="row">
                        ...
                        <input value="{{old('targetScore') ? : $challenge->target}}" id="targetScoreForm" class="form-control" style="width: 11%;" type="text"
                               name="targetScore" required="">
                    </div>
                    ...
                    <div class="row">
                        ...
                        <input value="{{old('wager') ? : $challenge->wager}}" class="form-control" style="width: 11%;" type="text" name="wager" id="wager" required="">
                    </div>
                    ...
                    <div class="row">
                        ...
                        <input class="form-control" style="width: 35%;" type="text" name="description" id="description"
                               required="">
                    </div>
                        ...
                        <div class="col-md-10" style="padding-top: 7px">
                            ...
                            @foreach($users as $user)
                                <label style="padding-right: 34px;">
                                    <input type="checkbox" name="challengers[]" id="usersOnChallenge"
                                           value="{{$user->id}}"/>
                                    <figure>
                                        <img class="img img-thumbnail btn-group" id="usersOnChallenge"
                                             src="{{ $user->picture }}" height="75" width="75">
